[README.md](https://github.com/srivera145/tetris/blob/master/README.md)
...
For Tetris, imagine that the board is a `two-dimensional array`. The square that can be moved is `shape (also a two-dimensional array) + coordinates`. The superposition of the board and the box is composed of the final result of `Matrix`. The properties above are built by `Immutable.js`, through its comparison method, you can easily write `shouldComponentUpdate`. Source Code:[/src/components/matrix/index.js#L35](https://github.com/srivera145/tetris/blob/master/src/components/matrix/index.js#L35)
...
Source Code:[/src/unit/music.js](https://github.com/srivera145/tetris/blob/master/src/unit/music.js). To achieve ajax loading mp3, and to WAA, control the playback process.
...
* Experience:
	* Press the arrow keys to move vertically and horizontally. The trigger frequency is different, the game can define the trigger frequency, instead of the original event frequency, the source code:[/src/unit/event.js](https://github.com/srivera145/tetris/blob/master/src/unit/event.js) ;
...
* Redux in the `store` not only connect to the method passed to `container`, you can jump out of the component, in other documents out to do flow control (dispatch), the source code:[/src/control/states.js](https://github.com/srivera145/tetris/blob/master/src/control/states.js)；
...
### multi-language
In the [i18n.json](https://github.com/srivera145/tetris/blob/master/i18n.json) is the configuration for the multi-language environment. You can change the language by passing the url parameter `lan` like this: `https://srivera145.github.io/tetris/?lan=en`
### Build
```
npm run build
```
